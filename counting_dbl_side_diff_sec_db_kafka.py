...
def count_people_between_lines(objects):
    ...
    for (objectID, cp) in objects.items():
        ...
        if frameHeight // 4 < cp[1] < frameHeight * 3 // 4 and objectID not in countedPersons:
            ...
            send_to_kafka(objectID, "In", crowd, "")
        ...
        # If there is a trackable object, utilize it to determine direction
            ...
            if not to.counted:
                ...
                if direction > 0 and cp[1] in range(frameHeight * 3//4 - 10, frameHeight * 3//4 + 10):
                    ...
                    send_to_kafka(objectID, "Out", crowd, totalDown, time_difference)  # Write object details to CSV file
...
def send_to_kafka(objectID, direction, crowd, position, time_difference=None):
    ...
    message = {
        ...
        "store_area": "A_02",  # shelf area identifier
        "zone": "shelf_area",   # for DB routing
        "IN": people,
        "OUT": totalDown
    }

    if time_difference:
        message["time_difference_sec"] = time_difference.total_seconds()
        message["time_difference_str"] = str(time_difference)
        message["frames_num"] = message["time_difference_sec"] * 1.35
        message["actual_time"] = message["frames_num"] / 30
